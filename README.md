...
* x1 представление:
  ...
  2. Мастер страница _Layout.cshtml
...
* Про существующие по умолчанию в шаблоне ASP.NET + MVC + WEB api подробно не буду:
  1. ~\Content\ - .css файлы (bootstrap + Site.css)
...
* Модели (директория: ~\Models)
  ...
  * EncryptionResultModel  
  ...
  Имеет всего 2 свойства:
    * bool IsError - ответ, была ли ошибка в процессе работы с данными  
  ...
  * HomeController  
  ...
  Методы:  
    * public ActionResult Index()  
    Возвращается представление Index.cshtml.
  * EncryptorController  
  Класс, отвечающий за обработку клиент-серверных запросов. Работает с обоими имеющимися моделями. Своего представления не имеет (использует Index.cshtml).  
    * enum:
    ...
    Содержит набор возвращаемых методами состояний - ошибок. Каждый элемент имеет текстовое описание через атрибут Description.  
    * Поля:
      * private const string _cookieName - имя сессионного cookie для хранения имени возвращаемого файла с результатом шифрования.
    ...
    * Вспомогательные методы:
    ...
    Пытается выполнить шифрование текста с обработкой ошибок. Возвращает ErrorMsg с результатом операции.
      * TryEncryptDocx  
    Пытается открыть и обработать .docx документ. Готов к поврежденным(corrupted) файлам Возвращает ErrorMsg с результатом операции.
      * TryEncryptTxt
    Выполняет попытку обработки текстового файла и борьбу с кодировками (Если кодировка не юникод, то есть в тексте появились нераспознанные символы '?', читает файл в кодировке Encoding.Default). Возвращает ErrorMsg с результатом операции.
* Дополнительные классы (директория: ~\Library)
  * VigenereEncryptor  
  ...
  устанавливает тип обработки (зашифровать/расшифровать) и кодовое слово.
      * нестатичный Encrypt(in string text)  
  шифрует фрагмент текста. Сохраняет следующий индекс в массиве индексов символов кодового слова. Позволяет работать с раздробленным на фрагменты текстом, например, с абзацами в .docx. В случае неправильного кодового слова возвращает null.
      * статичный Encrypt(in string text, in string keyWord, Operation op)  
  Шифрует текст целиком. Не подходит для раздробленного текста. В случае неправильного кодового слова возвращает null.
  ...
  Выполняет проверку на соответствие кодового слова заданным требованиям.
  * OldFileCleaner  
    Асинхронно через заданные интервалы времени удаляет из заданной директории файлы, существующие дольше какого-то интервала времени. Используется для очистки файлов в ~ \App_Data\ResultFiles.  
    ...
    * Clean()  
    В бесконечном цикле через время _cleanInterval запускает проверку директории на наличие файлов старше, чем _outdatingInterval и удаляет их.
    * ClearAll()  
...
* ~\Global.asax  
Кроме стандартных действие по конфигурации приложения на запуске, работает с экземпляром класса OldFileCleaner. Задает ему интервал очистки и время устаревания файлов.
...
Это наш фронтэнд.  
Index.cshtml содержит разметку страницы.  
Index.js содержит следующие компоненты:
  ...
  * Функции:
    * $(document).ready()  
    Выполняет инициализацию после полной загрузки страницы. Присваивает начальное значение переменной selectedTab и задает обработчик для событий переключения вкладок (текст или файл) и выбора файла.
    ...
    * function SendData(senderName)  
    При нажатии на кнопку "Зашифровать" или "Расшифровать" запускается эта функция. Она проверяет валидность ключевого слова и расширения файла, затем формирует ajax запрос к методу EncryptionController\DoCryptography. Запрос формируется в формате, соответствующем InputDataModel.  
      * В случае успеха запроса: проверяется ответ (в формате EncryptionResultModel). Если ответ сообщает об ошибке обработки (IsError == true) - показывает ее содержимое через alert. Если обработка была успешна (IsError == false) - выводит текст в окно предпросмотра и активирует кнопку скачивания результата.
      * В случае ошибки - сообщает в alert, что с сервером проблемы.

## Проект юнит-тестов

В Директории ~\TestFiles находятся тестовые файлы.
